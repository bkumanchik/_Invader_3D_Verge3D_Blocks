...
var game_over, button_left_pressed, button_right_pressed, button_fire_pressed, cam_type, debug_mode, invader_blast_pause, invader_been_hit, score, turret_exploding, turret_been_hit, lives, turret_check_x, fire_pressed, invader_fired, shot_check_y, turret_fired, shot_x, inv_shot_x, inv_dir, shot_y, inv_shot_y, increasing__score, pew_sound, explosion_sound, boom_sound, inv_x, inv_y;
...
function initialize() {
  ...
  setObjTransform('invader_root', false, 'position', [0, 3.22, ''], false);
  setObjTransform('turret_root', false, 'position', [0.098, -2.96, ''], false);
  ...
  if (!game_over) {
    registerSetTimeout(3, function() {
      if (!invader_been_hit) {
        if (!invader_fired) {
          invader_fired = true;
          inv_shot_x = getObjTransform('invader_root', false, 'position', 'x') + 0.09;
          inv_shot_y = getObjTransform('invader_root', false, 'position', 'y') - 0.7;
          setObjTransform('inv_shot', false, 'position', [inv_shot_x, inv_shot_y, ''], false);
...
function inv_exp_reset() {
  if (invader_been_hit) {
    ...
    registerSetTimeout(1, function() {
      setObjTransform('invader_root', false, 'position', [mathRandomInt(-2.8, 2.8), 3.25, ''], false);
      invader_fired = false;
      invader_been_hit = false;
      invader_blast_pause = false;
      animate_move_invader();
    ...
    if (getGamepadProp(0, 'CONNECTION_STATUS', '0')) {
      if (getGamepadProp(0, 'BUTTON', '0') && !fire_pressed) {
        ...
        if (!turret_fired) {
          ...
          setObjTransform('shot', false, 'position', [shot_x, shot_y, ''], false);
...
registerEveryFrame(function() {
  if (turret_fired) {
    ...
    shot_y = getObjTransform('shot', false, 'position', 'y');
    if (shot_y > 3) {
      changeVis('shot', false);
      shot_x = getObjTransform('turret_root', false, 'position', 'x');
      shot_y = getObjTransform('turret_root', false, 'position', 'y');
...
registerEveryFrame(function() {
  if (invader_fired && !game_over) {
    ...
    shot_check_y = getObjTransform('inv_shot', false, 'position', 'y');
    if (shot_check_y < -3.3) {
      changeVis('inv_shot', false);
      setObjTransform('inv_shot', false, 'position', [0, 5, ''], false);
      invader_fired = false;
      invader_fire();
    }
  }
